...
Emby 的歌单是**按用户存的**——不同账号可以有同名歌单，而音乐仓鼠用管理员 API key 能看到所有人的歌单。
不设限的话，它可能把歌加进**别人账号**的同名歌单里：文件确实同步了，但你在自己的客户端里看不到（实测踩到过）。

**对策**：「连接容器」页选一台 Emby/Jellyfin，在「播放列表归属用户」里选中**你自己用的那个账号**。
之后音乐仓鼠只在这个账号的歌单里匹配与创建——新建的歌单是共享类型，所有账号都能看到。

> 留空 = 不限制（匹配所有用户的歌单），只建议单用户服务器这么用。
