...
- [二、功能一览](#二功能一览)
- [三、界面](#三界面)
- [四、快速开始（Docker 部署）](#四快速开始docker-部署)
  - [方式一：docker run（最简）](#方式一docker-run最简)
  - [方式二：docker compose（推荐）](#方式二docker-compose推荐)
  - [方式三：自己构建（可选）](#方式三自己构建可选)
- [五、通知](#五通知)
- [六、行为说明（几个容易困惑的边界情况）](#六行为说明几个容易困惑的边界情况)
- [七、开发运行](#七开发运行)
- [八、项目结构](#八项目结构)
- [九、致谢与借鉴](#九致谢与借鉴)
...
- **通知**：同步跑完 / 出错时推一条到 **飞书 · 企业微信 · 钉钉 · Bark · Server酱 · Telegram · 自定义 Webhook**（7 种渠道）；**每个渠道自己选订阅哪些事件**（比如"失败推手机、成功只发群里"）；发送失败只记日志，不影响同步
...
在「设置 → 通知」里配：**7 种渠道**（按组分在横向子标签里切换），
**每个渠道单独勾「订阅哪些事件」** —— 比如配两个飞书群：一个只订"失败"、一个订"全部"。

| 分组 | 渠道 | 要填什么 |
|---|---|---|
| 群机器人 | 飞书 / 企业微信 / 钉钉 | 各自群机器人的 Webhook（钉钉/飞书开了「加签/签名校验」再把密钥填上） |
| 手机推送 | Bark | 服务器地址（默认官方）+ Key |
| | Server酱 | SendKey |
| | Telegram | Bot Token + chat_id（**国内直连不通**时把 API 地址换成自建反代） |
| 通用 | 自定义 Webhook | URL + GET/POST + 自定义请求头 + body 模板（`{title}` `{text}` 占位） |

每个渠道都有「**保存启用**」「停用」「发送测试」三个按钮 —— 「保存启用」会保存当前标签的内容
并把该渠道打开（总开关自动跟着变，界面上没有单独的全局开关）。企业微信有 2048 字节上限，超长会自动截断并提示。
